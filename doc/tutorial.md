...
can compare against (as long as you don't leak it outside). This can be handy if
you e.g. want to implement `last`/`count` on a vector/list, but don't want to
hit the out of bounds exception.
